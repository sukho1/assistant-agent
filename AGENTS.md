...
- **是** → A 类，按 counseling 框架执行完整流程（框架见下）。
- **否**（纯信息交流：IT 技术、日常技巧、事实查询、简单问候等）→ B 类，LLM 直接回复，不套用框架、不调用任何 skill。
- **拿不准** → 一律算 A 类。
- **日记目录缺失即跳过**：若项目根目录不存在 `diary/` 文件夹，则跳过所有日记检索/日记分析步骤，其余流程照常执行。

**首次打开加载 counseling**：会话首次打开本文件时，读取 `.codex/skills/counseling/SKILL.md` 全文并常驻上下文；此后 A 类分析直接引用该框架，不再重复读取该文件（仅当上下文压缩导致内容缺失时才补读一次）。

**重要区分——"执行 counseling 流程"与"读取文件"是两件事**：
- **counseling 流程（不可跳过）**：A 类消息的**每一轮**都必须完整执行 counseling 分析流水线（输入分析→策略→四维扫描→子skill路由→输出自检）。即便是同一会话的第 2、3、N 轮，这条路径每轮必走，不可因"之前执行过"而省略。该框架已随首次打开加载并常驻上下文，直接引用即可。
- **文件读取（可复用）**：skill 文件、知识库文章、用户档案如果在当前上下文窗口中已存在，不必重复读取——直接引用上下文中的内容即可。但这不影响 counseling 流程的强制性。
