...
- **MCP 返回 warming_up 时**：`eta_s` 已由服务端动态计算（model 阶段 ≈ 20s 总耗时，chromadb 阶段 ≈ 24s）。用 `eta_s` 作为等待秒数重试 MCP，min(eta_s+3, 15)s 防抖。重试后 `elapsed_s` 在涨说明预热推进中，正常 1-2 次重试命中。
- **MCP 返回 error（预热超时 >60s）**：异常——可能模型缓存损坏。可用 `mcp__diary-rag__search_diary` 返回的 `message` 中的测试命令诊断，但不要切 Bash 回退（同一个进程/线程，同样不可用）。
- **仅当 MCP 多次重试仍失败、或工具完全不在列表中时，才走 Bash 回退**。

### 日记 MCP 热加载

`.mcp.json` 配置了 `python diary_rag/server.py`，Claude Code 在会话启动时将其作为子进程拉起。进程启动后，`server.py` 立即在后台线程中加载 embedding 模型（BAAI/bge-small-zh-v1.5，从本地缓存加载约 15s）和 ChromaDB（约 4s），同时 MCP 握手在 2 秒内完成。
...
- **MCP 返回 warming_up 时**：读 `elapsed_s` + `stage` 算剩余等待时间，重试 MCP（不切 Bash）
- **仅当 MCP 重试多次仍失败或工具不在列表中时**，才使用 Bash 回退
