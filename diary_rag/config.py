...
# ONNX runtime files (fast path). The PyTorch SentenceTransformer remains a
# fallback only when these files or onnxruntime/tokenizers are unavailable.
BGE_DIR = os.path.join(DATA_DIR, "bge-zh")
ONNX_MODEL_PATH = os.path.join(BGE_DIR, "model.onnx")
TOKENIZER_PATH = os.path.join(BGE_DIR, "tokenizer.json")
