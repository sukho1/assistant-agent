...
**前置（每轮必做，先于输入分析）**：检查 `ma-zhuang/pending/` 目录，非空则先 flush——按登记文件执行 trace 落盘与 profile 更新，然后才进入本轮的输入分析。
...
**重要对话结束时**：若本次对话产生了实质性诊断信息（维度变化、综合体变化、关键事件、新认知框架），**只向 `ma-zhuang/pending/` 写登记文件**（trace 本体 + profile 更新范围），不在此轮同步写档案；**无实质性变化（纯分享、日常日记、信息交流）则跳过 profile-update，不机械更新。**

trace 使用精简模板（≤30 行要点式），回复前仅写登记文件，实际落盘在下次会话开头的 flush 阶段执行，不阻塞用户看到回应。
