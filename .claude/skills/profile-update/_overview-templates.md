...
> 事实与分析分离、更新内容约束见 SKILL.md。

## Overview 特有规则

- Overview 是**快照覆写式**更新——有变化的时间层级覆写，无变化则保持
- 上层周期（年/季）只在结构性变化时修订，不机械每周重写
- 下层周期的变化若构成上层结构性变化，向上传导
...
四个 overview 文件（comprehensive + dim1/dim2/dim3）共用此骨架，仅当前快照字段不同。
...
---
## Agent 分析
（默认不写；仅用户重点认同的分析作为重要事实写入，标注用户认同）
...
### 五综合体动态（comprehensive 专属，仅用户重点认同的分析时写）
